...
/** n倍帧缓冲 */
export class FrameBufferXN implements FrameBuffer {
  /** 帧缓冲数据 */
  ...
  constructor(width: number, height: number, samples: number) {
    ...
    this.frameBuffer = new Float32Array(this.width * this.height * 4 * this.samples);
  }
  ...
  setColor(x: number, y: number, color: Vec4, index: number = 0): void {
    let baseIndex = (x + y * this.width) * 4 * this.samples;
